...
func TestReadCommandHappy(t *testing.T) {
	cases := []struct {
		...
		{
			name:  "PUB with WAIT",
			input: "PUB orders - - 5 WAIT 2 500\nhello\n",
			want:  PubCommand{Topic: "orders", Payload: []byte("hello"), WaitReplicas: 2, WaitTimeoutMs: 500},
		},
		{
			name:  "PUB with DELAY and WAIT",
			input: "PUB orders - - 5 DELAY 100 WAIT 1 250\nhello\n",
			want:  PubCommand{Topic: "orders", Payload: []byte("hello"), DelayMs: 100, WaitReplicas: 1, WaitTimeoutMs: 250},
		},
		{
			name:  "PUB with WAIT before DELAY",
			input: "PUB orders - - 5 WAIT 1 250 DELAY 100\nhello\n",
			want:  PubCommand{Topic: "orders", Payload: []byte("hello"), DelayMs: 100, WaitReplicas: 1, WaitTimeoutMs: 250},
		},
		{
			name:  "INFO default section",
			input: "INFO\n",
			want:  InfoCommand{Section: "replication"},
		},
		{
			name:  "INFO replication",
			input: "INFO replication\n",
			want:  InfoCommand{Section: "replication"},
		},
		{
...
func TestReadCommandErrors(t *testing.T) {
	cases := []struct {
		...
		{"PUB WAIT missing timeout", "PUB orders - - 5 WAIT 2\nhello\n", ErrInvalidCommand},
		{"PUB WAIT non-numeric n", "PUB orders - - 5 WAIT two 500\nhello\n", ErrInvalidCommand},
		{"PUB WAIT negative n", "PUB orders - - 5 WAIT -1 500\nhello\n", ErrInvalidCommand},
		{"PUB WAIT non-numeric timeout", "PUB orders - - 5 WAIT 2 soon\nhello\n", ErrInvalidCommand},
		{"PUB unknown trailing token", "PUB orders - - 5 FOO 1\nhello\n", ErrInvalidCommand},
		{"PUB DELAY non-numeric", "PUB orders - - 5 DELAY soon\nhello\n", ErrInvalidCommand},
		{"PUB DELAY missing ms", "PUB orders - - 5 DELAY\nhello\n", ErrInvalidCommand},
		{"INFO unknown section", "INFO garbage\n", ErrInvalidCommand},
		{"INFO too many args", "INFO replication extra\n", ErrInvalidCommand},
